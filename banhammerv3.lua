-- Script Combinado: Interfaz del Mouse + Funcionalidad del Martillo
...
-- Variables de estado
local enabled = true
local mouseEquipped = false

-- Configuración del anillo de choque
...
-- Funciones de daño y etiquetado
	...
	if (obj ~= notme) then
		if (obj.className == "Part") or (obj.className == "Seat") then
			if (not obj.Anchored) and (((pos - obj.Position) * Vector3.new(1, 0, 1)).magnitude < 96) and (pos.y <= obj.Position.y + 8) and (pos.y >= obj.Position.y - 8) then
				delay((pos - obj.Position).magnitude / 96, function()
					doDamage(obj)
					obj.Velocity = ((obj.Position - pos).unit + Vector3.new(0, 0.5, 0)) * 96 + obj.Velocity
					obj.RotVelocity = obj.RotVelocity + Vector3.new(obj.Position.z - pos.z, 0, pos.x - obj.Position.x).unit * 40
				end)
			end
...
-- Manejo del clic del mouse (del primer script)
function onButton1Down(mouse)
	...
	-- Ejecutar ataque cuando se hace clic
...
-- Activación alternativa (tecla o botón de activación)
